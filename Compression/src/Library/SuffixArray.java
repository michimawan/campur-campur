...
public class SuffixArray {
    ...
    /**
     * Returns the suffix array keys by the index
     * @param i an integer between 0 and <em>N</em>-1
     * @return the array key of the index
     * @return -1 if not found
     */
    public int getArrayKeyByIndex(int index) {
    	for(int i=0; i<suffixes.length; i++)
    		if(index == suffixes[i].index)
    			return i;
    ...
    /**
     * Initializes a reverse index to key in suffix array 
     * for the given <tt>text</tt> string.
     */
    private void generateIndextoKeys() {
    	indexToKeys = new int[suffixes.length];
    	for(int i=0; i<suffixes.length; i++) {
    		indexToKeys[index(i)] = i;
    	}
    }
