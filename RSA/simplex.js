...
var theTabel = new makeArray2 (1,1);

var theStringTabel = new makeArray2 (1,1); 	// to display steps in the computation
...
while  ( (negIndicator) && (TabelNumber <= maxSteps) ) // phase 2
...
// reads problem and sets up the first tabel
...
// make the tabel .. note that all the variables are assumed to appear in the objective!!!
...
theTabel = new makeArray2 (numRows,numCols);

theStringTabel = new makeArray2 (numRows,numCols); // for display purposes
...
for (var j = 1; j <= numCols; j++) theTabel[numRows][j] = 0; // init

for (var i = 1; i <= numVariables; i++)

	{

	if (maximization) theTabel[numRows][i] = -eval(OBJ[i]);

	else theTabel[numRows][i] = eval(OBJ[i]);

	}

theTabel[numRows][numCols-1] = 1;

theTabel[numRows][numCols] = 0;
...
// alert("left-side of ineuqulaity = " + twoPart[1]);
 ...
 // alert(twoPart[2]);
	...
	theTabel[i][numCols] = eval(twoPart [2]); 		// the right-hand side

	if (GTE) {
		theTabel[i][numVariables + i] = -1;
		starred[i] = 1;
		phase1 = true;
			}

	else theTabel[i][numVariables + i] = 1;
...
// alert(count)
		if ((count == 1) && (roundSigDig(theTabel[theRowx][j],10)> 0))
		 	 {
...
var theString = "Tabel #" + TabelNumber + cr;
...
if (integerMode) theStringTabel = makeInteger(theTabel, RowNum, ColNum,true);
...
// alert("tabel is set up");
		...
		if (okToRoll)
			...
			theTabel = simplexMethod(theTabel,numRows,numCols);
	...
	// compute the expression
		...
		document.theSpreadsheet.expr.value = "Hasil optimalisasi";
